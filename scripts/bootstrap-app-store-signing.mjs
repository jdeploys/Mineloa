...
const distributionId = process.env.DISTRIBUTION_CERTIFICATE_ID
const distribution = await api(`/v1/certificates/${distributionId}`)
const installer = await api('/v1/certificates', {
  method: 'POST',
  body: JSON.stringify({
    data: {
      type: 'certificates',
      attributes: {
        ...
        csrContent: process.env.INSTALLER_CSR_CONTENT,
      },
    },
  }),
})
...
const profile = await api('/v1/profiles', {
  ...
  body: JSON.stringify({
    data: {
      type: 'profiles',
      attributes: {
        name: `Mineloa Mac App Store CI ${Date.now()}`,
        profileType: 'MAC_APP_STORE',
      },
      relationships: {
        bundleId: { data: { type: 'bundleIds', id: bundleId.id } },
        certificates: {
          data: [{ type: 'certificates', id: distributionId }],
        },
      },
    },
  }),
...
await writeFile('signing-assets/metadata.json', JSON.stringify({
  distributionCertificateId: distributionId,
